...
- **คืออะไร:** จังหวัดที่**น้ำท่วมอยู่จริงตอนนี้** ตัดสินจากสถานีวัดระดับน้ำของ สสน. (~1,100 สถานี · ตัวเลขจริงดู `stations_total` ใน `data/flood-status.json` ต้นทางเพิ่ม/ลดสถานีเรื่อยๆ) — ต่างจาก layer "พื้นที่เสี่ยงน้ำท่วม" ที่คำนวณจาก*พยากรณ์ฝน* อันนั้นคือน้ำที่*อาจจะ*มา อันนี้คือน้ำที่*วัดได้แล้ว*
- **เกณฑ์สี (จากสถานีวัดล้วน):**
  - 🔴 **ท่วม** — มีสถานีระดับ 5 (ล้นตลิ่ง) ≥1 จุด
  - 🟠 **ใกล้ล้นตลิ่ง** — สถานีระดับ 4 (น้ำมาก) ≥3 จุด **และ** ≥30% ของสถานีในจังหวัดนั้น
  - เงื่อนไข "หลายจุด + สัดส่วน" กันไม่ให้สถานีเดียวโดดๆ ลากทั้งจังหวัดเป็นสีท่วม · วัดกับข้อมูลจริง 28 ส.ค. 69 ได้ **14/77 จังหวัด** และกลุ่มเจ้าพระยาที่ออกมา (อยุธยา ลพบุรี สุพรรณบุรี สมุทรปราการ กทม.) **ตรงกับรายชื่อที่ ปภ. เตือนวันก่อนหน้าพอดี**
- **ข่าวเป็นบทสรุปประกอบเท่านั้น — ไม่มีผลกับสีบนแผนที่** (Google News RSS ถามแยกรายจังหวัด ย้อนหลัง 3 วัน) เหตุผล: พาดหัวหลอกได้ · 27 ส.ค. 69 ข่าว "ชัยภูมิวิกฤตแล้ง นาข้าวตายนับแสนไร่" มีคำว่า "น้ำท่วม" อยู่ในเนื้อหา ถ้าจับคำตรงๆ จะขึ้นสีว่าชัยภูมิท่วม ทั้งที่วิกฤตจริงคือตรงกันข้าม · และ query รวมคำว่า "น้ำท่วม" คืนข่าวต่างประเทศ (เนปาล/ทิเบต) มาเกือบทั้งหมด
- **จังหวัดที่ไม่ขึ้นสี = ไม่มีสถานีเข้าเกณฑ์ ไม่ใช่ยืนยันว่าไม่ท่วม** — น้ำท่วมขังนอกลำน้ำหรือพื้นที่ที่ไม่มีสถานีวัด ระบบนี้มองไม่เห็น · `valueFn` จึงคืน `null` ไม่ใช่ `0`
- **ข้อมูล:** ต่อยอดจาก `data/water-level.json` ที่ดึงมาแล้ว (ThaiWater API เดิม) — ไม่ยิง API เพิ่ม
- **สคริปต์:** `scripts/fetch_flood_status.py` (ต่อท้าย `fetch_water_level.py` ใน workflow เดียวกันทุก 3 ชม. — ถ้าแยก job จะอ่านไฟล์สถานีที่เก่ากว่า)

#### ทำไมเลิกใช้ GISTDA (ถอดออก 28 ส.ค. 69)
เคยมี layer นี้จากภาพดาวเทียม GISTDA WFS มาสามวัน แล้วเอาไปเทียบกับข่าวจริง — ไม่ผ่าน:
- **ภาพค้าง** — ต้นทางไม่ระบุวันถ่ายภาพเลยสักฟิลด์ ตรวจ 28 ส.ค. พบเป็นฉากเดิมค้างมา 3 วัน
- **พลาดเหตุการณ์ใหญ่สุดของช่วงนั้น** — น่านน้ำป่าถล่มจนเป็นข่าวทั้งประเทศ แต่ไม่มีในข้อมูลเลย เพราะน้ำป่าลดก่อนดาวเทียมจะถ่ายรอบถัดไป
- **ขึ้นสีผิดทาง** — ชัยภูมิถูกแสดงว่าท่วม ขณะที่ข่าวรายงานว่ากำลังวิกฤต*แล้ง*
ตัวเลข "นาข้าวจมน้ำ" ที่คำนวณด้วย GEE ถูกถอดไปพร้อมกัน เพราะมันแม่นได้ไม่เกินพื้นที่น้ำท่วมที่เป็นต้นทาง
...
- **คำนวณ (เกณฑ์):**
  ...
  เกณฑ์น้ำท่วม **ไม่ใช่ค่าคงที่ทั้งประเทศ** — เทียบฝน 7 วัน (พยากรณ์ **หรือ** ดาวเทียม อันที่สูงกว่า)
  กับ **ฝนปกติรายสัปดาห์ของเดือนนั้น เฉพาะจังหวัดนั้น** (ค่าปกติ 5 ปี 2021–2025):

  | ระดับ | เกณฑ์ |
  |-------|-------|
  | 🔴 เสี่ยงสูง | ≥ **3.0 เท่า** ของฝนปกติรายสัปดาห์เดือนนั้น |
  | 🟠 เฝ้าระวัง | ≥ **2.0 เท่า** |
  | 🟡 ระวัง | ≥ **1.5 เท่า** |
  | 🏜 เสี่ยงแล้ง | ฝนดาวเทียม < **5 มม./7วัน** + เขื่อน < **40%** |
  | 💧 น้ำต้นทุนน้อย | เขื่อน < **30%** |

  ตัวอย่างเกณฑ์ 🟡 เดือน ก.ย.: แม่ฮ่องสอน 276 มม. · นครราชสีมา 157 มม. · นครศรีธรรมราช 115 มม.
  · ค่าปกติคิดแบบเดียวกับฝั่งพยากรณ์ทุกขั้น: p90 ข้ามจุดตัวอย่าง ≤6 จุด **ของแต่ละวัน** แล้วค่อยรวม
  (ถ้าใช้ centroid จุดเดียว จะเอนไปทางเตือนเกินค่ากลาง 1.38 เท่า แกว่ง 1.00-2.54 เท่าแล้วแต่จังหวัด ·
  ถ้าใช้ p90 แต่รวมยอดรายจุดก่อน ฐานจะต่ำไปราว 21%)
  (จังหวัดที่ไม่มีค่าปกติใช้ค่าคงที่ 160/110/80 มม. แทน — ปัจจุบัน 0/77 จังหวัดใช้เส้นทางนี้)

- **คิดมาอย่างไร:** รวม "คาดการณ์ + ของจริง + น้ำต้นทุน" ให้เห็นความเสี่ยงรอบด้าน · เลือกค่าที่แย่ที่สุด (worst-case)
  · **ทำไมต้องเทียบรายเดือน ไม่ใช่ค่าคงที่หรือค่าเฉลี่ยทั้งฤดู:** 120 มม./สัปดาห์ เป็นเรื่องธรรมดาที่ตราด
  แต่มากผิดปกติที่นครราชสีมา และแม้แต่จังหวัดเดียวกันก็ต่างกันตามเดือน — ค่าปกติแม่ฮ่องสอนคือ
  ก.ย. 115 มม. แต่ พ.ย. 21 มม. · เกณฑ์คงที่ 120 มม. (ใช้ถึง 20 ส.ค. 69) และเกณฑ์ที่หารยอดฤดูกาล
  ด้วย 26 สัปดาห์แบนๆ (ใช้ถึง 11 ก.ย. 69) ทั้งคู่ทำให้ **เตือนราว 90% ของประเทศทุกวัน**
