...
- **🌾 พื้นที่นาที่จมน้ำ (คำนวณเพิ่มเอง):** ตัด polygon น้ำท่วม × mask นาข้าว (ชุดเดียวกับ layer สภาพนาข้าว) ใน GEE ที่ scale 250 ม.
  - **วัดความน่าเชื่อถือก่อนเปิดใช้** (25 ส.ค. 69): GEE สุ่มพิกเซลได้ **98.4%** ของพื้นที่ที่ GISTDA แจ้ง (ที่ 1 กม. ได้ 94.1% จึงเลือก 250 ม.) · ผลนิ่งข้าม scale (49.4% vs 46.1%) · สัดส่วนนากระจาย **18.9–74.6%** ตามภูมิศาสตร์จริง (มหาสารคาม/เพชรบูรณ์สูง · หนองคาย/อุดรฯ ต่ำ) — ถ้า mask ไม่มีข้อมูลค่าจะเกาะกลุ่มกัน
  - **⚠️ เป็นค่าประมาณและน่าจะต่ำกว่าจริง** — mask ชุดนี้จับพื้นที่นาได้ราว **0.70 เท่า**ของตัวเลข สศก. (ดูหมวดทราบปัญหาใน CHANGELOG) · ติดป้าย `rice_kind: "derived"` แยกจากพื้นที่น้ำท่วมที่เป็น `observed` · หน้าเว็บเขียน "ประมาณ" และ "ไม่ใช่ตัวเลขสำหรับเคลมชดเชย" เสมอ
  - **คำนวณเฉพาะตอน GISTDA ปล่อยฉากใหม่** (ดูจาก scene fingerprint) เพราะ GEE ใช้เวลา ~5 นาที · ฉากเดิมใช้ค่าเดิม · ถ้า GEE พังจะยังได้ข้อมูลน้ำท่วมตามปกติ แค่ `rice_rai: null` (ไม่เดาค่าแทน)
- **สคริปต์:** `scripts/fetch_gistda_flood.py` (ทุก 6 ชม. · ต้องมี `GEE_SERVICE_ACCOUNT_KEY` เฉพาะส่วนคำนวณนา)
