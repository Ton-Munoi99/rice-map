...
# Map ชื่อจังหวัดภาษาไทย → ชื่อภาษาอังกฤษที่ใช้ใน rice-data.js — ย้ายไป riceutils.py แล้ว
PROVINCE_MAP = PROVINCE_TH_EN
...
def main():
    ...
    # ── จำนวนจังหวัด: PDF ที่แกะได้ไม่ครบจะเขียนทับเหลือไม่กี่จังหวัด (ปกติ 29)
    prev_n = len(existing.get("provincial_prices", {}).get("data", {}))
    if prev_n and len(prices) < prev_n * 0.5:
        print(f"[ERROR] แกะได้ {len(prices)} จังหวัด (รอบก่อน {prev_n}) — ไม่บันทึก")
        return
