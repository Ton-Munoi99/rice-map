...
EXCLUDE = [
    ...
    "ขาสั้น", "โกงสอบ", "ภท.", "สส.สีน้ำเงิน", "ส.ส.สีน้ำเงิน",         # ดราม่าการเมือง ใช้คำว่า "ชาวนา" เชิงวาทศิลป์ ไม่ใช่ข่าวเกษตร
]

# ตัดทิ้งถ้า title ขึ้นต้นด้วยป้ายหมวดข่าวการเมือง (บางสำนักติด prefix นี้แม้เนื้อหาไม่เกี่ยวเกษตรเลย)
EXCLUDE_PREFIX = ["การเมือง -", "การเมือง:", "ข่าวการเมือง -"]
...
def main():
    ...
    for it in items:
        ...
        if any(bad in title for bad in EXCLUDE):
            continue
        if any(title.startswith(p) for p in EXCLUDE_PREFIX):
            continue
        key = norm(title)
        if key in seen:
            continue
