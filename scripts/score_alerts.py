#!/usr/bin/env python3
"""
Alert scoreboard — วัดความแม่นของ **พยากรณ์ฝน 7 วัน** เทียบฝนจริงจากดาวเทียม GSMaP

⚠️ ชื่อไฟล์ชวนเข้าใจผิด: **ไม่ได้วัดคุณภาพของระดับเตือนภัย** ใน agri-warnings.json
วัดแค่ว่าพยากรณ์ฝนแม่นไหมที่ไม้บรรทัดคงที่ (ดู HEAVY_MM/ANY_MM) · precision ต่ำที่นี่
แปลว่า "พยากรณ์ฝนหนักแล้วไม่ตกจริงบ่อย" ไม่ใช่ "เกณฑ์เตือนภัยตั้งผิด" — อย่าเอาไปอ้าง
ตัดสินเกณฑ์ของ layer
...
# ⚠️ สองค่านี้ **ไม่ใช่** เกณฑ์ของ layer เตือนภัย และตั้งใจไม่ผูกกัน
# layer ใช้เกณฑ์รายจังหวัด-รายเดือน (ทวีคูณของฝนปกติเดือนนั้น) ซึ่งต่างกันทุกจังหวัด
# ทุกเดือน เอามาเป็นเกณฑ์ให้คะแนนไม่ได้ · ที่นี่ต้องการ "ไม้บรรทัดคงที่" เพื่อให้
# rolling score เทียบข้ามเวลาได้ ถ้าผูกกับค่าคงที่ของ layer แล้ว layer ถูกจูนเมื่อไหร่
# ประวัติคะแนนจะเพี้ยนทันที (คอมเมนต์เดิมเขียนว่า "เท่า FLOOD_HIGH_MM/FLOOD_LOW_MM"
# ซึ่งค้างมาตั้งแต่ 4 ก.ย. 69 ที่ค่าพวกนั้นเปลี่ยนเป็น 160/80 — ไม่เท่ากันมาตั้งแต่ตอนนั้น)
HEAVY_MM  = 120   # ไม้บรรทัด "ฝนหนัก" สำหรับให้คะแนนพยากรณ์ (มม./7 วัน)
ANY_MM    = 30    # ไม้บรรทัด "ฝนมีนัย" สำหรับให้คะแนนพยากรณ์
    ...
    result = {
        "_meta": {
            ...
            "note": ("วัดความแม่นพยากรณ์ฝน 7 วัน เทียบฝนจริงดาวเทียม GSMaP · "
                     f"heavy = ≥{HEAVY_MM}มม., any = ≥{ANY_MM}มม. (ไม้บรรทัดคงที่ "
                     "ไม่ใช่เกณฑ์ของ layer เตือนภัย ซึ่งเป็นรายจังหวัด-รายเดือน) · "
                     "จับคู่เฉพาะช่วงวันที่ตรงกันเป๊ะ · **ไม่ได้วัดคุณภาพของระดับเตือนภัย**"),
            "heavy_mm": HEAVY_MM,
            "any_mm": ANY_MM,
        },
